#!/usr/bin/env python3
"""Fetch ALL medicine requests from MedBasket API — standalone script."""
...
def main():
    ...
    print("Fetching medicine requests from api.medbasket.com ...")
    ...
    # Save JSON
    json_path = os.path.join(OUT_DIR, "medicine_requests.json")
    ...
    # Save CSV
    csv_path = os.path.join(OUT_DIR, "medicine_requests.csv")
    all_keys = set()
    flat = []
    for r in records:
        fr = flatten(r)
    ...
    print(f"Done — {len(records):,} medicine requests saved.")
